...
namespace SistemaGestionOfertas.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        /// <summary>
        /// Convierte un objeto de tipo <see cref="User"/> en un objeto <see cref="UserDto"/>.
        /// </summary>
        /// <param name="user">Objeto <see cref="User"/> que contiene los datos del usuario original.</param>
        /// <returns>Un objeto <see cref="UserDto"/> con los datos mapeados del usuario.</returns>
        ...
        public IActionResult Edit(UserDto userDto)
        {
            if (ModelState.IsValid)
            {
                var user = userRepository.GetUsers().FirstOrDefault(x => x.Id == userDto.Id);
                if (user == null)
                {
                    return NotFound();
                }
